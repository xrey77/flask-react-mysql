...
export default function Profile() {
    ...
    const activateOTP = async (e: any) => {
        ...
        await api.put('/api/activateotp?id=' + idno,data, {headers: {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
             await api.get(`/api/getuserbydi?id=${idno}`, {headers: {
                Authorization: `Bearer ${token}`
            }})
            .then(async res => {
                    if (res.data.statuscode === 200) {
                        ...
                        QRCode.toDataURL(res.data.user.qrcodeurl, { errorCorrectionLevel: 'H' }, function (err, url) {
                            setQrcodeurl(url);
                          })
                        } else {
                            setQrcodeurl("");
    ...
    const submitProfile = async (e: any) => {
        ...
        await api.put('/api/updateuser?id=' + idno,data, {headers: {
            Authorization: `Bearer ${token}`}})
            .then(res => {
                if (res.data.statuscode === 200) {
                    setMsg(res.data.message);
    ...
    const chagePicture = async (e: any) => {
        ...
        api.post('/api/updateuserpicture?id=' + idno,formData, {
            headers: {
                    ...
                    <div className="col">
...

                        {
                          totp !== false ? 
                            ...
                            :
                                <img className="qrcode2" src="http://127.0.0.1:5000/assets/images/qrcode.png" alt="QRCODE"/> 
